...
history <- model %>% fit(
  ...
  #                                           filepath = paste0(getwd(), "/models/modelweights"))) 
)
...
save_model_weights_hdf5(model, paste0(getwd(), "/models/modelweights"))
...
# ggsave(paste0("plots/", elem, "_stochastic_holdout_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 45, height = 45, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
ggsave(paste0("plots/", elem, "_national_map_full_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 183, height = 225, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
ggplot(predgrid) + geom_raster(aes(x = x, y = y, fill = get(elem)), interpolate = FALSE) + theme_bw() + coord_equal() +
  ...
  theme(legend.justification = c(0, 0.5), legend.position = "none", legend.background=element_blank(),
        axis.text.y = element_text(angle = 90, vjust = 0.5, hjust=0.5)) +
  scale_y_continuous(breaks = seq(350000,450000, length.out = 3), labels = paste(seq(350000,450000, length.out = 3)), expand = c(0,0))
ggsave(paste0("plots/", elem, "_stoch_local_map_third_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 61, height = 72, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
ggsave(paste0("plots/", elem, "_stoch_local_map_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 136, height = 136, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
ggsave(paste0("plots/", elem, "_stoch_local_map_double_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 272, height = 272, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
#
...
ggsave(paste0("plots/", elem, "_local_map_with_points_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 136, height = 60, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
ggsave(paste0("plots/", elem, "_cross_section_2_", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 183, height = 72, units = "mm", dpi = 300, scale = 1.375)