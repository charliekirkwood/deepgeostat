...
conv_output <- conv_input %>%
  ...
  layer_conv_2d(filter = 64, kernel_size = c(3,3), dilation_rate = 1, strides = 3) %>%
  layer_activation("relu") %>%
  layer_spatial_dropout_2d(rate = dropratespat) %>%
  
  layer_conv_2d(filter = 64, kernel_size = c(3,3), dilation_rate = 1, strides = 1) %>%
  layer_activation("relu") %>%
  layer_spatial_dropout_2d(rate = dropratespat) %>%
  ...
  layer_conv_2d(filter = 64, kernel_size = c(2,2), dilation_rate = 1, strides = 1) %>%
  layer_activation("relu") %>%
  layer_spatial_dropout_2d(rate = dropratespat) %>%
...
i <- 1
for(i in 1:nsim){
  model <- dropout_sample_2(model = model)
