# Code to accompany the paper "Bayesian deep learning for spatial interpolation in the presence of auxiliary information" in Mathematical Geosciences.
# Authored by C Kirkwood, T Economou, N Pugeault, and H Odbert
# Contact c.kirkwood@exeter.ac.uk with any queries
# Code also available at https://github.com/charliekirkwood/deepgeostat
# For academic research purposes, readers may request access to the geochemical dataset we have used here 
# from the British Geological Survey at https://www.bgs.ac.uk/enquiries/home.html or by email to enquiries@bgs.ac.uk
# Otherwise, replace 'gchem' with point-sampled observation data for any target variable of choice
...
gchem <- fread("data/geochemistrydata.csv")
...
holdout <- data.frame(obs = y_test, preds = predict(meanmodel, x_test)[,1])
...
covtest <- melt(as.data.table(data.frame(ID = 1:length(y_test), y = y_test, x = drop(replicate(250, dropout_sample(x_test, batch_size = 4096))))), id.vars = c("ID", "y"))[order(ID)]
covtest
...
ggsave(paste0("plots/", elem, "_calibration_curve_interval", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 89, height = 89, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
ggsave(paste0("plots/", elem, "_calibration_curve_quantile", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 89, height = 89, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
ggplot() +
  ...
  labs(x = paste0("log(",elem,")"), y = "Density",
...
ggsave(paste0("plots/", elem, "_distribution_comparison", round(cor(holdout$preds, holdout$obs)^2, 2)*100, ".png"), width = 89, height = 89, units = "mm", type = "cairo", dpi = 300, scale = 1.375)
...
predgrid[, paste(elem) := apply(drop(replicate(250, dropout_sample(list(predimgs_ann, predloc_ann), batch_size = 16384))), 1, function(x) sd(x)), ]
...
for(i in 1:nsim){
  ...
  preds <- predgrid
  preds[, simpred := as.numeric(dropout_sample(list(predimgs_ann, predloc_ann), batch_size = 4096)[,1]), ] # predict with aleatoric uncertainty from model
  preds[, run := i]
  preds
...
for(i in 1:nsims){
  ...
  simdat[, simpred := as.numeric(dropout_sample(x_test, batch_size = 4096)[,1]), ] # predict with aleatoric uncertainty from model
...
ggplot() + 
  ...
  geom_line(data = variogramdata[type == "simulations"], aes(x = dist, y = gamma, group = interaction(type, sim), col = sim), alpha = 0.33, size = 0.5) +
  geom_line(data = variogramdata[type == "observations"], aes(x = dist, y = gamma, group = interaction(type, sim)), col = "#e41a1c", size = 1) +
  ...
  labs(x = "Distance (km)", y = "Semivariance", col = "sim") +
  theme(legend.justification = c(1, 0), legend.position = c(1, 0), legend.background=element_blank(), 
        axis.text.y = element_text(angle = 90, vjust = 0.5, hjust=0.5), legend.title.align=0)
