...
dropout_sample_2 <- function(model){
  ...
  sampl5 <- rep(rbinom(dim(dropweights[[9]])[4], 1, 1-dropratespat))
  sampl6 <- rep(rbinom(dim(dropweights[[11]])[4], 1, 1-dropratespat))
  ...
  maskl5 <- array(rep(sampl1, each = 9), dim = dim(dropweights[[9]])) # generate dropout mask for kernel and bias of layer 5
  maskl6 <- array(rep(sampl1, each = 4), dim = dim(dropweights[[11]])) # generate dropout mask for kernel and bias of layer 6
  
  # normal dropout masks
  maskl7 <- rbinom(dim(dropweights[[13]])[2], 1, 1-dropratedense) # generate dropout mask for kernel and bias of layer 5
  maskl8 <- rbinom(dim(dropweights[[15]])[2], 1, 1-dropratedense) # generate dropout mask for kernel and bias of layer 6
  maskl9 <- rbinom(dim(dropweights[[17]])[2], 1, 1-dropratedense) # generate dropout mask for kernel and bias of layer 7
  ...
  dropweights[[9]] <- (dropweights[[9]] * maskl5)*(1/(1-dropratespat))
  dropweights[[10]] <- (dropweights[[10]] * sampl5)*(1/(1-dropratespat))
  
  dropweights[[11]] <- (dropweights[[11]] * maskl6)*(1/(1-dropratespat))
  dropweights[[12]] <- (dropweights[[12]] * sampl6)*(1/(1-dropratespat))
  ...
  dropweights[[15]] <- t(t(dropweights[[15]]) * maskl8)*(1/(1-dropratedense))
  dropweights[[16]] <- (dropweights[[16]] * maskl8)*(1/(1-dropratedense))
  
  dropweights[[17]] <- t(t(dropweights[[17]]) * maskl9)*(1/(1-dropratedense))
  dropweights[[18]] <- (dropweights[[18]] * maskl9)*(1/(1-dropratedense))
