...
opt <- optimizer_lamb(learning_rate = 0.0025,
                      beta_1 = 0.9,
                      beta_2 = 0.999,
                      epsilon = 1e-6)
opt <- tf$keras$mixed_precision$LossScaleOptimizer(inner_optimizer = opt)
...
k_set_learning_phase(1)
predgrid[, paste(elem) := apply(drop(replicate(100, predict(model, list(predimgs_ann, predloc_ann), batch_size = 16384))), 1, function(x) sd(x)), ]
